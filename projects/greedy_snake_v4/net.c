...
*/
static int _loop_io(int fd, const void *buf, size_t len, int send_flag)
{
    const char *p = (const char *)buf;
    size_t left = len;
    while (left > 0)
    {
        ssize_t n = send_flag ? send(fd, p, left, 0) : recv(fd, (void *)p, left, 0);
        if (n > 0)
        {
            left -= n;
            p += n;
        ...
        else /* n < 0 */
        {
            ...
            if (errno == EWOULDBLOCK || errno == EAGAIN)
                return (int)(len - left); /* 非阻塞下暂时写/读不进去，返回已完成字节 */
            return -1;              /* 其他错误 */
        }
    }
    return (int)len; /* 全部完成 */
